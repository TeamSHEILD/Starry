...
coin = [
    ...
    "HA! The coin fell off the table! No one wins! (or loses....)"
]
...
async def on_ready():
	...
	await client.change_presence(
	    ...
	    activity=discord.Activity(
	        type=discord.ActivityType.playing, name=" star+assist"))
...
async def parrot(ctx, *, mssg=None):
	if mssg == None:
		...
		embed.add_field(
		    name="Slight Problem...", value='hmm whattya want me to copy?')
		await ctx.send(embed=embed)
	else:
...
async def serverpop(ctx):
	...
	embed.add_field(
	    name='How many members are in this server? (bots included)',
	    value=
	    f'There are {ctx.guild.member_count} members (including bots) in this server'
	)
	await ctx.send(embed=embed)
...
async def feedback(ctx, *, mssg=None):
	if mssg == None:
		...
		embed.add_field(
		    name="Slight Problem...", value='hmm whattya want to feedback on?')
		await ctx.send(embed=embed)
	else:
		...
		embed.add_field(
		    name=f'''{ctx.author} from {ctx.guild.name} says
    ''',
		    value=f'{mssg}')
...
async def ping(ctx):
	...
	embed.add_field(
	    name="📡",
	    value=f'This is my ping -> ({round(client.latency * 10000)})ms')
	await ctx.send(embed=embed)
...
async def afk(ctx, *, mssg=None):
	if mssg == None:
		...
		embed.add_field(
		    name="Your AFK Status has been set to:", value=f"'{mssg}'")
		await ctx.send(embed=embed)
...
async def human(ctx, *, mssg=None, amount=1):
	if mssg == None:
		...
		embed.add_field(
		    ...
		    value="I can't ghost your message if you don't give me one!")
		await ctx.send(embed=embed)
	else:
...
async def about(ctx):
	...
	embed.add_field(
	    name="I am Serenity's favored creation 😋",
	    value=
...
async def time(ctx):
	embed = discord.Embed(
	    title="⌚ Time in Coordinated Universal Time", color=0x575353)
...
async def invite(ctx):
	embed = discord.Embed(
	    title="Invite 𝑺𝒕𝒂𝒓𝒓𝒚! 🥳",
	    description=
	    "Heyyyy thank you sooo much, for requesting to invite me to your server 🥰! It will be a great pleasure of mine to bring that small sprinkle of comedy to your channel, and you won\'t regret this!😋\n𝑺𝒕𝒂𝒓𝒓𝒚 ⎰:sparkles:⎱ with administrator permissions: https://discord.com/api/oauth2/authorize?client_id=742030317451214888&permissions=8&scope=bot\n𝑺𝒕𝒂𝒓𝒓𝒚 ⎰:sparkles:⎱ with only text and general permissions (no administrator): https://discord.com/api/oauth2/authorize?client_id=742030317451214888&permissions=2084568823&scope=bot",
	    color=0x8F40FF)
	embed.set_footer(
	    text=f"Thank you {ctx.author}", icon_url=ctx.author.avatar_url)
	await ctx.send(embed=embed)
...
async def swapcase(ctx, *, mssg=None):
	if mssg == None:
		await ctx.send(
		    f'{ctx.author.mention} re-run the command and also type in something that you would like to swapcase'
		)
	else:
...
async def lower(ctx, *, mssg=None):
	if mssg == None:
		await ctx.send(
		    f'{ctx.author.mention} re-run the command and also type in something that you would like to all lowercase'
		)
	else:
...
async def upper(ctx, *, mssg=None):
	if mssg == None:
		await ctx.send(
		    f'{ctx.author.mention} re-run the command and also type in something that you would like to uppercase'
		)
	else:
...
async def join(ctx, *, mssg=None):
	if mssg == None:
		await ctx.send(
		    f'{ctx.author.mention} re-run the command and also type in something that you would like to join characters to'
		)
	else:
...
async def creator(ctx):
	...
	embed.set_footer(
	    text=f"{ctx.author} took interest", icon_url=ctx.author.avatar_url)
	embed.add_field(
	    name="Keep in note that the creator changes names sometimes...",
	    value="Currently Right Now: 'Serenity#7879'")
	embed.add_field(
	    name="Previous name",
	    value="'StarryAurora🌟#8018','Shaaaaddiii✨#8018','Shadi#7879'")
	await ctx.send(embed=embed)
...
async def allinfo(ctx, member: discord.Member):
	...
	embed.add_field(
	    name='Discriminator:', value=f'{member.discriminator}', inline=False)
...
#---------------------------------------
anime = [
    'Soo cute 😋', 'Those noses....', 'Soooo Kawaii!', 'Awwwwwwwww',
    'Those manga eyes....'
]
animecolor = [
    0xff79bf, 0xff91dc, 0xe486ff, 0xffa7e1, 0xff2586, 0xff58a2, 0xd358ff
]
...
async def kawaii(ctx):
	...
	embed = discord.Embed(
	    title="Pure Kawaii 🥰", description=animee, color=colora)
...
async def turtle(ctx):
	...
	embed = discord.Embed(
	    title="Here is a Turtle", description='Lil Turtle', color=t3)
...
async def frap(ctx):
	...
	embed = discord.Embed(
	    title="Here is a homemade Frappuccino",
	    description='Very emaculate',
	    color=frap2)
...
async def aesthetic(ctx):
	...
	embed = discord.Embed(
	    title="Aesthetics!", description='Sooo cool', color=aa1)
	embed.set_footer(
	    text=
	    "All credits to **CharlieC#0666** for allowing me to use their art for this! 🥰"
	)
...
async def memes(ctx):
	...
	embed = discord.Embed(
	    title="**DANK OR NOT**", description='Very memes', color=memes2)
...
async def assist(ctx):
	embed = discord.Embed(
	    title="🥳 𝑺𝒕𝒂𝒓𝒓𝒚 ⎰✨⎱ is here to have fun! 🥳",
	    description=
	    "Choose an option below! This is a bot to have fun with, so... have fun!! 😁🤣",
	    color=0x8F40FF)

	embed.set_footer(
	    text=f"Welcome to 𝑺𝒕𝒂𝒓𝒓𝒚 ⎰✨⎱'s interface, {ctx.author}!",
	    icon_url=ctx.author.avatar_url)

	embed.set_image(
	    url=
	    'https://media1.tenor.com/images/24b4cf8512e58420d0cdfea3df5a3cce/tenor.gif?itemid=14432583'
	)
...
async def mod(ctx):
	embed = discord.Embed(
	    title="𝑺𝒕𝒂𝒓𝒓𝒚 is here to MODERATE!",
	...
	embed.add_field(
	    name="🔨 Starry Moderation!",
	    value="So far, very few commands that require admin privelages",
	    inline=False)
...
async def text(ctx):
	embed = discord.Embed(
	    title="Welcome to the Text command module! 🥳",
	    description="Text can be modified per the commands below!",
	    color=0x6A9EFF)
	...
	embed.add_field(
	    name="💬 Text Commands!",
	    value=
	    "Here, you can make Starry say stuff, reverse stuff, humanize stuff, idk lol",
	    inline=False)

	embed.add_field(
	    name='Text Modifiers',
	    value=
	    'Starry will simply repeat what you say by modifying the text that you put in!'
	)
...
async def activity(ctx):
	embed = discord.Embed(
	    title="Perform a simple activity! 🥳",
	    description="Such as random generators, SUSHI.",
	    color=0xFF756A)
	...
	embed.add_field(
	    name="🎈 Activities!", value="Simple activities!", inline=False)
...
async def social(ctx):
	embed = discord.Embed(
	    title="Socialize with Starry! 🥳",
	    description="Tell a joke or greeting!",
	    color=0x6AFFC5)
	...
	embed.add_field(
	    name="🤗 Socialize!", value="Random test generation!", inline=False)
...
async def media(ctx):
	embed = discord.Embed(
	    title="Here is some Medias! 🥳",
	    description="The Kawaii command, I give credit to the OwO bot!",
	    color=0xFFFB6A)
	...
	embed.add_field(
	    name="📸 Media Sorts!",
	    value=
	    "You can do Kawaii! (inspired by the OwO bot!)\nOr you can see pictures and shots of a turtles I took myself!",
	    inline=False)
...
async def utility(ctx):
	embed = discord.Embed(
	    title="Some simple utility tools! 🥳",
	    description="Very simple commands, not moderation",
	    color=0x949494)
	...
	embed.add_field(
	    name="🔧 Starry Utility!", value="Simple tools", inline=False)
...
async def abocred(ctx):
	embed = discord.Embed(
	    title="Check my about, credits, updates, and invite me! 🥳",
	    color=0xFF9974)
	...
	embed.add_field(
	    ...
	    value="About, Credits, Invites, Update log",
	    inline=False)

	embed.add_field(
	    name="🔖Client ID (if you want to add me with customized permissions)",
	    value='719944711972061274',
	    inline=False)
